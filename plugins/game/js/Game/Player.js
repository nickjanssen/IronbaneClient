...
var Player = Fighter.extend({
  ...
  Tick: function(dTime) {
    ...
    if ( doCameraCheck && !cinema.IsPlaying()) {
      ...
      var intersects = terrainHandler.RayTest(ray, ironbane.camera.position);
    ...
    if ( !cinema.IsPlaying() ) {
      ...
      if ( this.cameraStatus != CameraStatusEnum.ThirdPerson ) {
        ironbane.camera.lookAtPosition.lerpSelf(this.position.clone().addSelf(new THREE.Vector3(Math.sin(radians), 1, Math.cos(radians))), dTime * 10);
      }
      else {
        ironbane.camera.lookAtPosition.lerpSelf(this.position.clone().addSelf(new THREE.Vector3(0, 1, 0)), dTime * 10);
      }
    ...
    for(var u=0;u<ironbane.unitList.length;u++) {
      ...
      if ( unit instanceof Fighter && unit.InRangeOfUnit(ironbane.player, 30) && unit.id < 0 ) {
        ...
        var intersects = terrainHandler.RayTest(ray,
          unit.position, false, false, distance);
...
//      }
  ...
  UpdateMouseProjectedPosition: function() {
    ...
    var intersects = terrainHandler.RayTest(ray, this.position, false, false, 20);
